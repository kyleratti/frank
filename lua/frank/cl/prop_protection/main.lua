...
hook.Add( "HUDPaint", "frank_PP_HUDPaint", function( )
	if( IsValid( LocalPlayer( ):GetActiveWeapon( ) ) && LocalPlayer( ):GetActiveWeapon( ):GetClass( ) == "gmod_camera" ) then return; end

	local objEnt = LocalPlayer( ):GetEyeTrace( ).Entity;
	if( !IsValid( objEnt ) || !objEnt:CPPIControlled( ) ) then return; end
