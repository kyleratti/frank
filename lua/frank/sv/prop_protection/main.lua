...
function ent:CPPISetOwner( objPl )
	...
	if( objPhys:IsValid( ) && !self:IsVehicle( ) ) then
		objPhys:SetMass( 1 );
	end
end
...
hook.Add( "OnPhysgunReload", "frank_PP_OnPhysgunReload", function( objWeapon, objPl )
	...
	if( !IsValid( objEnt ) ) then return false; end
...
function frank.PP.ClearProps( strUniqueID )
	for k,v in pairs( ents.GetAll( ) ) do
		if( v:CPPIControlled( ) && v:CPPIGetOwner( ) == strUniqueID ) then
...
hook.Add( "EntityRemoved", "frank_PP_EntityRemoved", function( objEnt )
	if( objEnt:CPPIControlled( ) && !objEnt.PP_Ignore ) then
...
-- nabbed from FPP
if( cleanup ) then
	frank.PP.OldCleanup = frank.PP.OldCleanup || cleanup.Add;

	function cleanup.Add( objPl, iType, objEnt )
		if( IsValid( objPl ) && IsValid( objEnt ) ) then
